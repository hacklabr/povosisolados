...
/********************************Add Post Popup ajax function**/
    add_action('wp_ajax_my_action', 'huge_it_video_gal_my_action_callback_frontend');
    add_action('wp_ajax_nopriv_my_action', 'huge_it_video_gal_my_action_callback_frontend' );
...
<script type="text/javascript">
                jQuery(document).ready(function($) {
                    ...
                    var response;
                     ...
                     $('input[name=pause_on_hover').change(function(){
                        if($('input[name=pause_on_hover').prop('checked')  == false){
                            $('input[name=pause_on_hover').val('off');
                        }
                        else if($('input[name=pause_on_hover').prop('checked')  == true){
                            $('input[name=pause_on_hover').val('on');
                        }
                    }); 
                    ...
                    jQuery('#hugeitvideogalleryinsert').on('click', function() {
                        ...
                        var huge_it_sl_effects = $('#huge_it_sl_effects').val();
                        var sl=huge_it_sl_effects;
                        var display_type = $('#videogallery-current-options-'+sl+' select[id="display_type"]').val();
                        var content_per_page = $('#videogallery-current-options-'+sl+' input[id="content_per_page"]').val();
                        var sl_width = $('input[name=sl_width]').val();
                        var sl_height = $('input[name=sl_height]').val();
                        var videogallery_list_effects_s = $('select[name=videogallery_list_effects_s]').val();
                        var sl_pausetime = $('input[name=sl_pausetime').val();
                        var sl_changespeed = $('input[name=sl_changespeed').val();
                        var sl_position = $('select[name=sl_position').val();
                        pause_on_hover = $('input[name=pause_on_hover]').val();
                        var data = {
                            video_id:                       id,
                            action:                         'my_action',
                    ...
                    $('#videogallery-unique-options').on('change',function(){
                        $(this).find( 'div[id^="videogallery-current-options"]').each(function(){
                            if(!$(this).hasClass( "active" )){
                                $(this).find('ul li input[name="content_per_page"]').attr('name', '');
                                $(this).find('ul li select[name="display_type"]').attr('name', '');
                            }else{
                                $(this).find('ul li input[id="content_per_page"]').attr('name', 'content_per_page');
                                $(this).find('ul li select[id="display_type"]').attr('name', 'display_type');
                            }
                        });
                        if($('select[name="display_type"]').val()== 2){
                            $('li[id="content_per_page"]').hide();
                    ...
                    jQuery('#huge_it_videogallery-select').change(function (){ 
                        ...
                        $.post("<?php echo admin_url('admin-ajax.php'); ?>", data, function(response){
                            ...
                            $('#huge_it_sl_effects').val(response.huge_it_sl_effects);
                            $('#huge_it_sl_effects').change();
                            $('select[name=display_type]').val(response.display_type);
                            $('input[name=content_per_page]').val(response.content_per_page);
                            $('input[name=sl_width]').val(response.sl_width);
                            $('input[name=sl_height').val(response.sl_height);
                            $('select[name=videogallery_list_effects_s]').val(response.videogallery_list_effects_s);
                            $('input[name=sl_pausetime').val(response.sl_pausetime);
                            $('input[name=sl_changespeed').val(response.sl_changespeed);
                            $('select[name=sl_position').val(response.sl_position);
                            ...
                            $('input[name=pause_on_hover').val(response.pause_on_hover);
                            if($('input[name=pause_on_hover').val()  == 'on'){
                                $('input[name=pause_on_hover').attr('checked','checked');
                            }
                            else $('input[name=pause_on_hover').removeAttr('checked');
...
<style>
    ...
    #videogallery-unique-options label{
        width: 145px;
    ...
    #videogallery-unique-options input[type='number']{
        width:79px;
    }
    #videogallery-unique-options input[type='text'],#videogallery-unique-options select{
        width:141px;
    }
    #huge_it_sl_effects{
        width:226px;
    }
    #videogallery-unique-options input[type='checkbox']{
        margin-left: 1px;
    }
    #TB_ajaxContent{
        overflow:visible;
